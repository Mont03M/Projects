...
namespace ChessGame.ChessBoard
{
    ...
    public class Board : ObservableObject
    {
        ...
        /*
        public void ResetChessSquares()
        {
            foreach(var square in ChessSquares_ ?? [])
            {
                if(square.ChessPiece_ != null)
                {
                    square.ChessPiece_ = null;
                }
            }
        }
        */
        ...
        /*
        public void TestIsCheckmateSetChessPieces()
        {
            WhiteChessPieces.Clear();
            BlackChessPieces.Clear();
            ...
            foreach (var square in ChessSquares_ ?? [])
            {
                if (square.ChessBoardLocation.Equals("g5"))
                {
                    var cp = PlaceTestChessPiece(square.boardLocation.X, square.boardLocation.Y, PieceType.KING, ChessPieceColors.BLACK, GetChessPieces(ChessPieceColors.BLACK));
                    square.ChessPiece_ = cp;
                    BlackChessPieces.Add(cp);
                }
                else if (square.ChessBoardLocation.Equals("h1"))
                {
                    var cp = PlaceTestChessPiece(square.boardLocation.X, square.boardLocation.Y, PieceType.KING, ChessPieceColors.WHITE, GetChessPieces(ChessPieceColors.WHITE));
                    square.ChessPiece_ = cp;
                    WhiteChessPieces.Add(cp);
                }
                else if (square.ChessBoardLocation.Equals("h2"))
                {
                    var cp = PlaceTestChessPiece(square.boardLocation.X, square.boardLocation.Y, PieceType.PAWN, ChessPieceColors.BLACK, GetChessPieces(ChessPieceColors.BLACK));
                    square.ChessPiece_ = cp;
                    BlackChessPieces.Add(cp);
                }
                else if (square.ChessBoardLocation.Equals("g3"))
                {
                    var cp = PlaceTestChessPiece(square.boardLocation.X, square.boardLocation.Y, PieceType.BISHOP, ChessPieceColors.BLACK, GetChessPieces(ChessPieceColors.BLACK));
                    square.ChessPiece_ = cp;
                    BlackChessPieces.Add(cp);
                }
        ...
        private ChessPiece? PlaceTestChessPiece(int i, int j, PieceType pieceType, ChessPieceColors pieceColor,
           Dictionary<PieceType, string> chessPieces)
        {
            return pieceType switch
            {
                PieceType.PAWN => new Pawn(ChessGameService, pieceType, pieceColor,
                                      [Special_Moves.EnPASSENT, Special_Moves.PAWN_PROMOTION],
                                      new ChessSquareLocation(i, j),
                                      chessPieces[pieceType]),
                PieceType.BISHOP => new Bishop(ChessGameService,
                                        pieceType,
                                        pieceColor,
                                        new ChessSquareLocation(i, j),
                                        chessPieces[pieceType]),
                PieceType.KNIGHT => new Knight(ChessGameService, pieceType, pieceColor,
                                        new ChessSquareLocation(i, j),
                                        chessPieces[pieceType]),
                PieceType.ROOK => new Rook(ChessGameService, pieceType, pieceColor,
                                       Special_Moves.CASLTING,
                                       new ChessSquareLocation(i, j),
                                       chessPieces[pieceType],
                                       CastlingSide.NOCASTERSIDE),
                PieceType.QUEEN => new Queen(ChessGameService, pieceType, pieceColor,
                                        new ChessSquareLocation(i, j),
                                        chessPieces[pieceType]),
                PieceType.KING => new King(ChessGameService, pieceType, pieceColor,
                                       Special_Moves.CASLTING,
                                       new ChessSquareLocation(i, j),
                                       chessPieces[pieceType]),
                _ => null,
